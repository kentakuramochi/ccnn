...
layer_t* fc(int in_n, int in_c, int in_h, int in_w, int out)
{
    layer_t* fc = layer_create();

    fc->type = LAYER_FC;

    fc->out = ndmat_create(in_n, out, 1, 1);
    fc->w   = ndmat_create(in_n, out, in_h, in_w);
    fc->b   = ndmat_create(in_n, out, 1, 1);
